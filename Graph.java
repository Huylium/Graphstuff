...
public abstract class Graph {
	...
	public List<Integer> degreeSequence() {
		...
		List<Integer> ramLethal = new ArrayList<Integer>();
		for(int i = 0; i<numVertices; i++) {
			TreeSet<Integer> vert = new TreeSet<Integer>();
			vert.addAll(getInNeighbors(i));
			vert.addAll(getNeighbors(i));
			ramLethal.add(vert.size());
		}

		//ramLethal.stream().sorted(Collections.reverseOrder());
		Collections.sort(ramLethal);
		Collections.reverse(ramLethal);



     /*for(int i = 0; i<ramLethal.size(); i++){
        boolean check = false;
        for(int j =0; j<ramLethal.size(); j++){
           if(ramLethal.get(i) >= ramLethal.get(j)){
              check = true;
           }
        }
        if(!check){
